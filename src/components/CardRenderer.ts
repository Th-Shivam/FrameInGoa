...
export async function renderIDCard(data: IDCardData): Promise<string> {
  ...
  drawCenteredText(ctx, data.name.toUpperCase(), NAME_CX, NAME_Y, NAME_MAX_W, 26, '700', '#F5EFE4')
  debugDot(ctx, NAME_CX, NAME_Y, `Name y=${NAME_Y}`, '#00ffff')

  // 4. Draw Role — cover the "BUILDER" placeholder with user's role
  // First paint a small background rect to cover the placeholder text
  ctx.fillStyle = '#0B3D22'
  ctx.fillRect(NAME_CX - 160, ROLE_Y - 28, 320, 36)
  drawCenteredText(ctx, data.role.toUpperCase(), ROLE_CX, ROLE_Y, ROLE_MAX_W, 20, '700', '#FF007F')
  ...
  drawCenteredText(ctx, data.hackerId, ID_CX, ID_Y, ID_MAX_W, 22, '700', '#FFE600')
